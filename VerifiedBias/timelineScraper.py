...
with open ("Datasets/" + inputFileName[14:-2] + ".csv", 'wb') as csvfile:
	...
	userCount = 0
	csvwriter.writerow(['User', 'Screen Name', 'Description', 'Location', 'Status Count', 'Created', 'Follower Count', 'First Tweet Date', 'Last Tweet Date', 'Time Between T1 Tn',
	...
	for user in userList:
		try:
			...
			csvwriter.writerow([tweepyUser.name.encode('utf-8'), tweepyUser.screen_name, tweepyUser.description.replace(";","").encode('utf-8'), tweepyUser.location.encode('utf-8'), tweepyUser.statuses_count, tweepyUser.created_at, tweepyUser.followers_count, firstTweetDate, lastTweetDate, days, float(userTweetCount)/days, userTweetCount, userRetweetTotal, userFavoriteTotal, float(userFavoriteTotal)/userTweetCount, float(userRetweetTotal)/userTweetCount, avgRet+avgFav, (avgFav+avgRet)/tweepyUser.followers_count])
			#print ("------")
			print userCount
